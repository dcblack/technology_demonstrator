...
const char* tlmx_command_to_str(tlmx_command_t s) {
  switch(s) {
    ...
    case TLMX_MUTEX_LOCK    : return "TLMX_DEBUG_WRITE";
    case TLMX_MUTEX_TRYLOCK : return "TLMX_MUTEX_TRYLOCK";
    case TLMX_MUTEX_UNLOCK  : return "TLMX_MUTEX_UNLOCK";
    case TLMX_INTERRUPT     : return "TLMX_INTERRUPT";
...
// Copy constructor
tlmx_packet::tlmx_packet(const tlmx_packet& rhs)
{
  this->id       = rhs.id;
...
int tlmx_packet::pack(char* buffer)
{
  ...
  memcpy( &buffer[ TLMX_DELAY_NS_INDEX ], &delay_ns, TLMX_PACKET_DELAY_NS_SIZE );
  memcpy( &buffer[ TLMX_TIME_NS_INDEX  ], &time_ns , TLMX_PACKET_TIME_NS_SIZE  );
  memcpy( &buffer[ TLMX_STRM_WID_INDEX ], &strm_wid, TLMX_PACKET_STRM_WID_SIZE );
...
int tlmx_packet::unpack(char* buffer)
{
  ...
  memcpy( &delay_ns , &buffer[ TLMX_DELAY_NS_INDEX ], TLMX_PACKET_DELAY_NS_SIZE  );
  memcpy( &time_ns  , &buffer[ TLMX_TIME_NS_INDEX  ], TLMX_PACKET_TIME_NS_SIZE   );
  memcpy( &strm_wid , &buffer[ TLMX_STRM_WID_INDEX ], TLMX_PACKET_STRM_WID_SIZE  );
...
void tlmx_packet::print(const char* message) const
{
  ...
  if (time_ns)  printf("@%lldns",time_ns);
  if (delay_ns) printf(":%lldns",delay_ns);
  printf("}\n");
}
...
int main(int argc, char* argv[])
{
  ...
  for (int i=0; i!=20; ++i) {
    ...
    p2 = new tlmx_packet( TLMX_IGNORE, 0, 0, &d2[0], 500, 42 );
    p2->print("new p2");
    p2->unpack(buffer);
